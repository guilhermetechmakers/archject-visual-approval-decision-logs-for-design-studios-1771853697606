...
function updateStep(steps, index, status) {
    const now = new Date().toISOString();
    const s = [...steps];
    if (s[index]) {
        s[index] = { ...s[index], status };
        if (status === 'in_progress')
            s[index].started_at = now;
        if (status === 'completed')
            s[index].finished_at = now;
    }
    return s;
}
export function runJobWorker(jobId) {
...
async function processJob(jobId) {
    ...
    await delay(500);
    if (getJobRow(jobId)?.status === 'CANCELLED')
        return;
