...
decisionsApiRouter.get('/decisions/:decisionId/confirmation', (req, res) => {
    const { decisionId } = req.params;
    const userId = optionalAuth(req);
    const row = db.prepare(`SELECT id, project_id, status, last_confirmed_at, last_confirmed_by, confirmation_reference_id, confirmed_context
     FROM decisions WHERE id = ?`).get(decisionId);
    if (!row) {
    ...
    if (row.last_confirmed_by) {
        try {
            lastConfirmedBy = JSON.parse(row.last_confirmed_by);
        }
        catch {
            // ignore
        }
