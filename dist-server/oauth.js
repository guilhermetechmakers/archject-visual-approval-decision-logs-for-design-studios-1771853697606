...
import { createAccessToken, createRefreshToken, storeRefreshToken, setRefreshTokenCookie, } from './auth-utils.js';
...
oauthRouter.get('/google/callback', async (req, res) => {
    try {
        ...
        const existingOAuth = db.prepare('SELECT user_id FROM oauth_accounts WHERE provider = ? AND provider_user_id = ?').get('google', providerUserId);
        if (existingOAuth) {
            const user = db.prepare('SELECT id, email, first_name, last_name FROM users WHERE id = ?').get(existingOAuth.user_id);
        ...
        const existingUser = db.prepare('SELECT id, email, first_name, last_name FROM users WHERE email = ?').get(email);
        if (existingUser) {
            db.prepare(`INSERT INTO oauth_accounts (id, user_id, provider, provider_user_id, provider_email)
         VALUES (?, ?, 'google', ?, ?)`).run(crypto.randomUUID(), existingUser.id, providerUserId, email);
        ...
        db.prepare(`INSERT INTO users (id, first_name, last_name, email, email_verified, password_hash, company)
       VALUES (?, ?, ?, ?, 1, NULL, NULL)`).run(userId, firstName, lastName, email);
        db.prepare(`INSERT INTO oauth_accounts (id, user_id, provider, provider_user_id, provider_email)
       VALUES (?, ?, 'google', ?, ?)`).run(crypto.randomUUID(), userId, providerUserId, email);
