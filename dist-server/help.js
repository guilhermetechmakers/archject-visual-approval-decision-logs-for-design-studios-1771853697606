...
helpRouter.get('/kb', (req, res) => {
    try {
        ...
        const countRow = db.prepare(`SELECT COUNT(*) as c FROM kb_articles WHERE ${where}`).get(...params);
        const rows = db.prepare(`SELECT id, slug, title, excerpt, tags, featured, updated_at, created_at
...
helpRouter.get('/kb/featured', (_req, res) => {
    try {
        const rows = db.prepare(`SELECT id, slug, title, excerpt, tags, updated_at
       FROM kb_articles WHERE published = 1 AND featured = 1
       ORDER BY updated_at DESC LIMIT 5`).all();
        const articles = rows.map((r) => ({
            id: r.id,
            slug: r.slug,
            title: r.title,
...
helpRouter.get('/kb/:slug', (req, res) => {
    try {
        const { slug } = req.params;
        const row = db.prepare('SELECT id, slug, title, excerpt, body, tags, author_id, updated_at, created_at FROM kb_articles WHERE slug = ? AND published = 1').get(slug);
        if (!row) {
...
helpRouter.get('/checklist', (req, res) => {
    try {
        ...
        const rows = db.prepare(`SELECT id, step_key, status, metadata, updated_at FROM onboarding_checklists
       WHERE ${where} ORDER BY step_key`).all(...params);
...
helpRouter.post('/checklist', (req, res) => {
    try {
        ...
        const existing = db.prepare('SELECT id FROM onboarding_checklists WHERE step_key = ? AND (team_id = ? OR (? IS NULL AND team_id IS NULL)) AND (user_id = ? OR (? IS NULL AND user_id IS NULL))').get(stepKey, teamId || null, teamId, userId || null, userId);
        ...
        db.prepare('INSERT INTO onboarding_checklists (id, team_id, user_id, step_key, status, updated_at) VALUES (?, ?, ?, ?, ?, ?)').run(id, teamId || null, userId || null, stepKey, status, now);
...
helpRouter.post('/uploads', upload.array('files', 5), (req, res) => {
    try {
...
helpRouter.post('/support/ticket', (req, res) => {
    try {
        ...
        db.prepare(`INSERT INTO support_tickets (id, user_id, project_id, name, email, subject, description, attachments, status, source, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'open', ?, ?, ?)`).run(id, null, projectId || null, name.trim(), email.trim(), subject.trim(), description.trim(), att, src, now, now);
        res.status(201).json({
            id,
            ticketId: id,
...
helpRouter.post('/support/report', (req, res) => {
    try {
        ...
        const att = Array.isArray(attachments) ? JSON.stringify(attachments) : null;
        try {
            db.prepare(`INSERT INTO support_reports (id, user_id, email, attempted_path, message, user_agent, attachments, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'new')`).run(id, userIdStr, emailStr, attemptedPath.trim(), msgStr, userAgentStr, att);
        }
        catch (e) {
            ...
            db.prepare(`INSERT INTO support_tickets (id, user_id, name, email, subject, description, status, source, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, 'open', 'help-form', datetime('now'), datetime('now'))`).run(id, userIdStr, name, emailStr ?? 'noreply@archject.local', subject, description);
        }
