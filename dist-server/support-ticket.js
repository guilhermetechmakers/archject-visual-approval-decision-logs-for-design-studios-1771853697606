 ...
 */
supportTicketRouter.post('/support/ticket', (req, res) => {
    try {
        ...
        db.prepare(`INSERT INTO support_tickets (ticket_id, incident_id, user_id, email, message, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, 'open', ?, ?)`).run(ticketId, incidentIdVal, userId, emailVal, msgVal, now, now);
        res.status(201).json({
            ticketId,
            status: 'open',
        });
