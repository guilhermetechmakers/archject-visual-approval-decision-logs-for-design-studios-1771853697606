...
decisionsRouter.get('/projects/:projectId/decisions', requireAuth, (req, res) => {
    ...
    let rows;
    try {
        rows = db.prepare(`SELECT id, project_id, title, status, created_at, updated_at, approved_option_id FROM decisions WHERE ${where} ORDER BY created_at DESC`).all(...params);
    }
    catch {
        try {
            ...
            rows = db.prepare(`SELECT id, project_id, title, status, created_at, COALESCE(updated_at, created_at) as updated_at FROM decisions WHERE project_id = ? AND (deleted_at IS NULL OR deleted_at = '') ORDER BY created_at DESC`).all(projectId);
        }
    }
    const decisionIds = rows.map((r) => r.id);
    let optionsByDecision = {};
    if (decisionIds.length > 0) {
        const placeholders = decisionIds.map(() => '?').join(',');
        const optRows = db.prepare(`SELECT id, decision_id, title, description FROM decision_options WHERE decision_id IN (${placeholders}) ORDER BY decision_id, sort_order`).all(...decisionIds);
        for (const o of optRows) {
            if (!optionsByDecision[o.decision_id])
                optionsByDecision[o.decision_id] = [];
...
decisionsRouter.get('/projects/:projectId/decisions/:decisionId', requireAuth, (req, res) => {
    ...
    let row;
    try {
        row = db.prepare(`SELECT id, project_id, title, description, status, created_at, updated_at, etag, version, approved_option_id, cloned_from_decision_id, version_number, is_archived FROM decisions WHERE ${whereClause}`).get(decisionId, projectId);
    }
    catch {
        try {
            row = db.prepare(`SELECT id, project_id, title, status, created_at, updated_at, etag, version, approved_option_id FROM decisions WHERE ${whereClause}`).get(decisionId, projectId);
        }
        catch {
    ...
    let options;
    try {
        options = db.prepare('SELECT id, title, description, is_default, is_recommended, sort_order, image_url, drawing_ids, cost_impact FROM decision_options WHERE decision_id = ? ORDER BY sort_order').all(decisionId);
    }
    catch {
        options = db.prepare('SELECT id, title, description, is_default, is_recommended, sort_order FROM decision_options WHERE decision_id = ? ORDER BY sort_order').all(decisionId);
    }
    res.json({
        id: row.id,
        ...
        options: options.map((o) => ({
            ...
            drawingIds: o.drawing_ids ? (() => { try {
                return JSON.parse(o.drawing_ids);
            }
            catch {
                return [];
            } })() : undefined,
...
decisionsRouter.post('/projects/:projectId/decisions/:decisionId/publish', requireAuth, (req, res) => {
    ...
    try {
        db.prepare('INSERT INTO client_tokens (id, project_id, decision_ids, scope, token_hash, expires_at, revoked) VALUES (?, ?, ?, ?, ?, ?, 0)').run(tokenId, projectId, JSON.stringify([decisionId]), 'read_approve', tokenHash, expiresAt);
    }
    catch (e) {
        if (String(e).includes('no such column')) {
            db.prepare('INSERT INTO client_tokens (id, project_id, decision_ids, scope, token_hash, expires_at) VALUES (?, ?, ?, ?, ?, ?)').run(tokenId, projectId, JSON.stringify([decisionId]), 'read_approve', tokenHash, expiresAt);
        }
        else
            throw e;
    }
...
decisionsRouter.post('/decisions/:decisionId/clone', requireAuth, (req, res) => {
    const userId = req.userId;
    const { decisionId: sourceId } = req.params;
    const { projectId } = req.body;
    const source = db.prepare('SELECT id, project_id, title, description, type, account_id FROM decisions WHERE id = ? AND (deleted_at IS NULL OR deleted_at = \'\')').get(sourceId);
    if (!source)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Decision not found' });
    ...
    try {
        db.prepare(`INSERT INTO decisions (id, project_id, account_id, title, type, status, created_at, updated_at, created_by, etag, version, cloned_from_decision_id, version_number)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, 1)`).run(newId, targetProjectId, source.account_id, source.title + ' (Copy)', source.type, 'draft', now, now, userId, etag, sourceId);
    }
    catch (e) {
        if (String(e).includes('no such column')) {
            db.prepare(`INSERT INTO decisions (id, project_id, account_id, title, type, status, created_at, updated_at, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(newId, targetProjectId, source.account_id, source.title + ' (Copy)', source.type, 'draft', now, now, userId);
        }
        else
            throw e;
    }
    let opts;
    try {
        opts = db.prepare('SELECT id, title, description, is_default, is_recommended, sort_order, image_url, drawing_ids, cost_impact FROM decision_options WHERE decision_id = ? ORDER BY sort_order').all(sourceId);
    }
    catch {
        opts = db.prepare('SELECT id, title, description, is_default, is_recommended, sort_order FROM decision_options WHERE decision_id = ? ORDER BY sort_order').all(sourceId);
    }
    for (let i = 0; i < opts.length; i++) {
        const o = opts[i];
        const optId = crypto.randomUUID();
        try {
            db.prepare(`INSERT INTO decision_options (id, decision_id, title, description, is_default, is_recommended, sort_order, image_url, drawing_ids, cost_impact)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(optId, newId, o.title, o.description ?? null, o.is_default, o.is_recommended, i, o.image_url ?? null, o.drawing_ids ?? null, o.cost_impact ?? null);
        }
        catch {
            db.prepare(`INSERT INTO decision_options (id, decision_id, title, description, is_default, is_recommended, sort_order)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).run(optId, newId, o.title, o.description ?? null, o.is_default, o.is_recommended, i);
        }
    }
    db.prepare(`INSERT INTO decision_audit_log (id, decision_id, action, performed_by, performed_at, details)
     VALUES (?, ?, ?, ?, ?, ?)`).run(crypto.randomUUID(), newId, 'cloned', userId, now, JSON.stringify({ fromDecisionId: sourceId }));
...
decisionsRouter.post('/decisions/:decisionId/archive', requireAuth, (req, res) => {
    const userId = req.userId;
    const { decisionId } = req.params;
    const row = db.prepare('SELECT id, project_id FROM decisions WHERE id = ? AND (deleted_at IS NULL OR deleted_at = \'\')').get(decisionId);
    if (!row)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Decision not found' });
    const now = new Date().toISOString();
    try {
    ...
    db.prepare(`INSERT INTO decision_audit_log (id, decision_id, action, performed_by, performed_at, details)
     VALUES (?, ?, ?, ?, ?, ?)`).run(crypto.randomUUID(), decisionId, 'archived', userId, now, '{}');
...
decisionsRouter.post('/projects/:projectId/decisions/:decisionId/share', requireAuth, (req, res) => {
    const { projectId, decisionId } = req.params;
    const row = db.prepare('SELECT id, status FROM decisions WHERE id = ? AND project_id = ? AND (deleted_at IS NULL OR deleted_at = \'\')').get(decisionId, projectId);
    if (!row)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Decision not found' });
...
decisionsRouter.get('/projects/:projectId/decisions/:decisionId/comments', requireAuth, (req, res) => {
    const { projectId, decisionId } = req.params;
...
decisionsRouter.post('/projects/:projectId/decisions/:decisionId/comments', requireAuth, (req, res) => {
    const userId = req.userId;
    const { projectId, decisionId } = req.params;
    ...
    const userName = db.prepare('SELECT first_name, last_name FROM users WHERE id = ?').get(userId);
    const authorName = userName ? `${userName.first_name} ${userName.last_name}`.trim() : 'User';
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
    try {
        db.prepare(`INSERT INTO portal_comments (id, decision_id, parent_comment_id, author_id, author_name, content, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, decisionId, parentCommentId ?? null, userId, authorName, text.trim(), now);
    }
    catch (e) {
