...
webhooksRouter.post('/payment-provider', (req, res) => {
    ...
    try {
        if (type === 'invoice.payment_succeeded' || type === 'invoice.paid') {
            ...
            if (invId) {
                db.prepare(`UPDATE invoices SET status = 'paid', amount_paid_cents = amount_due_cents, updated_at = ? WHERE provider_invoice_id = ?`).run(new Date().toISOString(), invId);
            }
        }
        else if (type === 'invoice.payment_failed' || type === 'payment_failed') {
            const subId = data?.object?.subscription ?? data?.subscriptionId;
            if (subId) {
                db.prepare(`UPDATE subscriptions SET status = 'past_due', updated_at = ? WHERE provider_subscription_id = ?`).run(new Date().toISOString(), subId);
            }
        }
    }
    catch (err) {
