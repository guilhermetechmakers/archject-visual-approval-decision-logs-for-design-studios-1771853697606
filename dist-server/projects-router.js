...
projectsRouter.get(PROJECTS_BASE, requireAuth, (req, res) => {
    ...
    const rows = db.prepare(`SELECT id, name, description, created_at, updated_at, deleted_at
     FROM projects
     WHERE ${where}
    ...
    const pendingByProject = db.prepare(`SELECT project_id, COUNT(*) as count FROM decisions
     ...
     AND status IN ('pending', 'in_review') GROUP BY project_id`).all();
...
projectsRouter.post(PROJECTS_BASE, requireAuth, (req, res) => {
    ...
    db.prepare(`INSERT INTO projects (id, name, description, account_id, studio_id, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?)`).run(id, name.trim(), description?.trim() ?? null, userId, 'default', now, now);
    res.status(201).json({
        id,
        name: name.trim(),
...
projectsRouter.get(`${PROJECTS_BASE}/:id`, requireAuth, (req, res) => {
    const { id } = req.params;
    const row = db.prepare(`SELECT id, name, description, created_at, updated_at, deleted_at
     FROM projects WHERE id = ?`).get(id);
    if (!row) {
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
    }
    const pendingCount = db.prepare(`SELECT COUNT(*) as count FROM decisions
     WHERE project_id = ? AND (deleted_at IS NULL OR deleted_at = "")
     AND status IN ('pending', 'in_review')`).get(id);
    res.json({
        id: row.id,
        name: row.name,
...
projectsRouter.get(`${PROJECTS_BASE}/:id/activity`, requireAuth, (req, res) => {
    const { id: projectId } = req.params;
    const limit = Math.min(20, Math.max(1, parseInt(req.query.limit || '10', 10)));
    const project = db.prepare('SELECT id FROM projects WHERE id = ?').get(projectId);
    if (!project)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
    const rows = db.prepare(`SELECT a.id, a.decision_id, a.action, a.performed_by, a.performed_at, d.title as decision_title
     ...
     ORDER BY a.performed_at DESC LIMIT ?`).all(projectId, limit);
    res.json({
        items: rows.map((r) => ({
            id: r.id,
...
projectsRouter.patch(`${PROJECTS_BASE}/:id`, requireAuth, (req, res) => {
    const { id } = req.params;
    const { name, description } = req.body;
    ...
    if (updates.length === 0) {
        const current = db.prepare('SELECT id, name, description, created_at, updated_at FROM projects WHERE id = ?').get(id);
        return res.json({
            id: current.id,
            name: current.name,
...
projectsRouter.delete(`${PROJECTS_BASE}/:id`, requireAuth, (req, res) => {
    const { id } = req.params;
    const row = db.prepare('SELECT id, deleted_at FROM projects WHERE id = ?').get(id);
    if (!row) {
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
...
projectsRouter.get(`${PROJECTS_BASE}/:id/client-links`, requireAuth, (req, res) => {
    const { id: projectId } = req.params;
    const project = db.prepare('SELECT id FROM projects WHERE id = ?').get(projectId);
    if (!project)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
    let rows;
    try {
        rows = db.prepare(`SELECT id, project_id, decision_ids, scope, expires_at, revoked, last_used_at, created_at
    ...
    const decisionIds = [...new Set(rows.flatMap((r) => {
            try {
                return JSON.parse(r.decision_ids);
            }
            catch {
                return [];
            }
    ...
    if (decisionIds.length > 0) {
        ...
        const decRows = db.prepare(`SELECT id, title FROM decisions WHERE id IN (${placeholders})`).all(...decisionIds);
...
projectsRouter.post(`${PROJECTS_BASE}/:id/client-links/:tokenId/revoke`, requireAuth, (req, res) => {
    const { id: projectId, tokenId } = req.params;
    const project = db.prepare('SELECT id FROM projects WHERE id = ?').get(projectId);
    if (!project)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
    const token = db.prepare('SELECT id, project_id, revoked FROM client_tokens WHERE id = ? AND project_id = ?').get(tokenId, projectId);
    if (!token) {
...
projectsRouter.get(`${PROJECTS_BASE}/:id/client-links/:tokenId/analytics`, requireAuth, (req, res) => {
    const { id: projectId, tokenId } = req.params;
    const project = db.prepare('SELECT id FROM projects WHERE id = ?').get(projectId);
    if (!project)
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
    const token = db.prepare('SELECT id, project_id FROM client_tokens WHERE id = ? AND project_id = ?').get(tokenId, projectId);
    if (!token) {
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Client link not found' });
    }
    let rows = [];
    try {
        rows = db.prepare(`SELECT event_type, decision_id, created_at FROM portal_analytics
       WHERE token_id = ? ORDER BY created_at DESC LIMIT 500`).all(tokenId);
    }
    catch {
...
projectsRouter.post(`${PROJECTS_BASE}/:id/restore`, requireAuth, (req, res) => {
    const { id } = req.params;
    const row = db.prepare('SELECT id, deleted_at FROM projects WHERE id = ?').get(id);
    if (!row) {
        return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' });
