...
function auditLog(actorId, actionType, targetType, targetId, metadata) {
    const id = crypto.randomUUID();
    db.prepare('INSERT INTO audit_logs (id, actor_id, action_type, target_type, target_id, metadata) VALUES (?, ?, ?, ?, ?, ?)').run(id, actorId, actionType, targetType, targetId, metadata ? JSON.stringify(metadata) : null);
}
...
privacyRouter.post('/v1/privacy/exports', authMiddleware, (req, res) => {
    try {
        ...
        const formatsArr = Array.isArray(formats) ? formats.filter((f) => validFormats.includes(f)) : ['json', 'csv', 'pdf'];
        const includeArr = Array.isArray(include) ? include.filter((i) => validInclude.includes(i)) : ['decisions', 'projects', 'comments', 'attachments'];
        const id = crypto.randomUUID();
        const expiresAt = new Date(Date.now() + EXPORT_EXPIRY_HOURS * 60 * 60 * 1000).toISOString();
        db.prepare(`INSERT INTO data_exports (id, user_id, status, formats, include_data, expires_at)
       VALUES (?, ?, 'requested', ?, ?, ?)`).run(id, userId, JSON.stringify(formatsArr), JSON.stringify(includeArr), expiresAt);
        ...
        db.prepare(`UPDATE data_exports SET status = 'processing', started_at = ? WHERE id = ?`).run(new Date().toISOString(), id);
        // Simulate completed export with a placeholder storage key (no actual S3)
        const storageKey = `exports/${userId}/${id}.zip`;
        db.prepare(`UPDATE data_exports SET status = 'completed', completed_at = ?, storage_key = ?, size_bytes = 0 WHERE id = ?`).run(new Date().toISOString(), storageKey, id);
...
privacyRouter.get('/v1/privacy/exports', authMiddleware, (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        const rows = db.prepare('SELECT id, status, formats, include_data, size_bytes, requested_at, completed_at, expires_at FROM data_exports WHERE user_id = ? ORDER BY requested_at DESC LIMIT 20').all(userId);
        const exports = rows.map((r) => ({
            id: r.id,
            status: r.status,
            formats: parseJson(r.formats) ?? [],
            include: parseJson(r.include_data) ?? [],
...
privacyRouter.get('/v1/privacy/exports/:id', authMiddleware, (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        const { id } = req.params;
        const row = db.prepare('SELECT * FROM data_exports WHERE id = ? AND user_id = ?').get(id, userId);
        if (!row)
        ...
        return res.json({
            ...
            include: parseJson(row.include_data) ?? [],
            size_bytes: row.size_bytes,
            requested_at: row.requested_at,
            completed_at: row.completed_at,
            expires_at: row.expires_at,
            error_message: row.error_message,
            presigned_url: row.status === 'completed' && !isExpired
...
privacyRouter.post('/v1/privacy/deletions', authMiddleware, async (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        ...
        const id = crypto.randomUUID();
        db.prepare(`INSERT INTO deletion_requests (id, user_id, status, verification_method, scheduled_for)
       VALUES (?, ?, 'scheduled', 'password', ?)`).run(id, userId, scheduledFor);
...
privacyRouter.get('/v1/privacy/deletions/:id', authMiddleware, (req, res) => {
    try {
        const userId = req.userId;
        if (!userId)
            return res.status(401).json({ message: 'Unauthorized' });
        const { id } = req.params;
        const row = db.prepare('SELECT * FROM deletion_requests WHERE id = ? AND user_id = ?').get(id, userId);
        if (!row)
...
privacyRouter.get('/v1/privacy/backups', authMiddleware, (req, res) => {
    try {
        // In production, check admin role. For now allow any authenticated user to see summary.
        const rows = db.prepare('SELECT id, job_timestamp, snapshot_key, retention_until, size_bytes, status FROM backups ORDER BY job_timestamp DESC LIMIT 10').all();
        return res.json({
            cadence: 'nightly',
            retention_days: 90,
            encryption: 'AES-256',
