...
function backfillSearchIndex() {
    try {
        ...
        // Projects
        const projects = db.prepare('SELECT id, name, created_at, updated_at FROM projects').all();
        const insertIdx = db.prepare(`INSERT OR REPLACE INTO search_index (id, document_id, type, title, snippet, content, project_id, status, created_at, updated_at)
       VALUES (?, ?, 'project', ?, ?, ?, ?, 'active', ?, ?)`);
        ...
        let decisions;
        try {
            decisions = db.prepare(`SELECT d.id, d.title, d.status, d.created_at, COALESCE(d.updated_at, d.created_at) as updated_at, d.project_id, d.reviewer_id, d.tags FROM decisions d`).all();
        }
        catch {
            decisions = db.prepare(`SELECT d.id, d.title, d.created_at as updated_at, d.status, d.created_at, d.project_id, d.reviewer_id, d.tags FROM decisions d`).all();
        }
        const insertDec = db.prepare(`INSERT OR REPLACE INTO search_index (id, document_id, type, title, snippet, content, project_id, status, assignee_id, tags, created_at, updated_at)
       VALUES (?, ?, 'decision', ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
        for (const d of decisions) {
            const id = `decision-${d.id}`;
            const content = [d.title, d.tags].filter(Boolean).join(' ');
            insertDec.run(id, d.id, d.title, d.title, content, d.project_id, d.status, d.reviewer_id, d.tags, d.created_at, d.updated_at ?? d.created_at);
        }
        // Templates
        const templates = db.prepare('SELECT id, name, description, type, tags_json, created_at, updated_at FROM templates_library WHERE is_deleted = 0').all();
        const insertTpl = db.prepare(`INSERT OR REPLACE INTO search_index (id, document_id, type, title, snippet, content, status, tags, created_at, updated_at)
       VALUES (?, ?, 'template', ?, ?, ?, 'published', ?, ?, ?)`);
        ...
        // Library files
        const files = db.prepare('SELECT id, filename, project_id, filetype, uploaded_at FROM library_files WHERE is_archived = 0').all();
        const insertFile = db.prepare(`INSERT OR REPLACE INTO search_index (id, document_id, type, title, snippet, content, project_id, status, created_at, updated_at)
       VALUES (?, ?, 'file', ?, ?, ?, ?, 'active', ?, ?)`);
        ...
        if (!String(e).includes('UNIQUE'))
            console.error('[DB] backfillSearchIndex:', e);
    }
}
