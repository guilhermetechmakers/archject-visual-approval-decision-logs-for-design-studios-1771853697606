 ...
 */
errorsRouter.post('/errors/report', (req, res) => {
    try {
        const { timestamp, userId: bodyUserId, route, method, url, requestId, errorMessage, stackTraceSummary, clientContext, tags, dedupToken, } = req.body;
        ...
        const ctxStr = clientContext != null && typeof clientContext === 'object'
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NULL)`).run(incidentId, correlationId, receivedAt, userId, routeVal, methodVal, urlVal, msgVal, stackVal, ctxStr, tagsStr);
        res.status(200).json({
            incidentId,
            receivedAt,
        });
