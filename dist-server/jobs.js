...
const JOB_TYPES = ['EXPORT_PDF', 'EXPORT_CSV', 'SIGNING', 'UPLOAD', 'GENERIC'];
const JOB_STATUSES = ['QUEUED', 'IN_PROGRESS', 'COMPLETED', 'FAILED', 'CANCELLED'];
...
function logJobHistory(jobId, actor, action, detail) {
    const id = crypto.randomUUID();
    db.prepare('INSERT INTO job_history (id, job_id, actor, action, detail) VALUES (?, ?, ?, ?, ?)').run(id, jobId, actor, action, detail ? JSON.stringify(detail) : null);
}
function runJobWorker(jobId) {
    const row = db.prepare('SELECT * FROM jobs WHERE id = ?').get(jobId);
    if (!row || row.type !== 'EXPORT_PDF' && row.type !== 'EXPORT_CSV' && row.type !== 'GENERIC')
        return;
    const steps = [
...
jobsRouter.post('/jobs', requireAuth, (req, res) => {
    ...
    db.prepare(`INSERT INTO jobs (id, project_id, user_id, type, payload, status, cancellable, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, 'QUEUED', ?, ?, ?)`).run(jobId, projectId ?? null, userId, type, JSON.stringify(payload ?? {}), cancellable ? 1 : 0, now, now);
    logJobHistory(jobId, 'USER', 'created', { type, projectId });
    runJobWorker(jobId);
    res.status(201).json({ jobId, status: 'QUEUED' });
});
jobsRouter.get('/jobs/:jobId', requireAuth, (req, res) => {
    const userId = req.userId;
    ...
    let resultUrls = row.result_urls ?? '[]';
    try {
        const urls = JSON.parse(resultUrls);
        resultUrls = JSON.stringify(urls.map((u) => ({
            ...u,
...
jobsRouter.post('/jobs/:jobId/cancel', requireAuth, (req, res) => {
    const userId = req.userId;
    const { jobId } = req.params;
    const row = db.prepare('SELECT * FROM jobs WHERE id = ?').get(jobId);
    if (!row) {
    ...
    const now = new Date().toISOString();
    db.prepare('UPDATE jobs SET status = ?, cancelled_at = ?, updated_at = ? WHERE id = ?').run('CANCELLED', now, now, jobId);
    ...
    const job = {
        ...
        result_urls: typeof updated.result_urls === 'string' ? JSON.parse(updated.result_urls) : (updated.result_urls ?? []),
        error: updated.error,
...
jobsRouter.get('/jobs', requireAuth, (req, res) => {
    const userId = req.userId;
    ...
    const rows = db.prepare(`SELECT * FROM jobs WHERE ${where} ORDER BY created_at DESC LIMIT ?`).all(...params, limit);
    const jobs = rows.map((r) => ({
        ...
        result_urls: typeof r.result_urls === 'string' ? JSON.parse(r.result_urls) : (r.result_urls ?? []),
...
jobsRouter.get('/jobs/:jobId/download', requireAuth, (req, res) => {
    const userId = req.userId;
    const { jobId } = req.params;
    const row = db.prepare('SELECT * FROM jobs WHERE id = ?').get(jobId);
    if (!row) {
