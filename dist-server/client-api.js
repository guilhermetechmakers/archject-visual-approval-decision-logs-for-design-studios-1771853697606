...
clientRouter.get('/client/:token', (req, res) => {
    ...
    const row = db.prepare(`SELECT d.id, d.project_id, d.title, d.status, d.created_at, d.updated_at
    ...
    let opts;
    try {
        const options = db.prepare(`SELECT id, label, description FROM decision_options WHERE decision_id = ?`).all(decisionId);
