...
export interface TokenExpiryNoticeProps {
  ...
  message?: string
  variant?: 'info' | 'warning' | 'error' | 'revoked'
  className?: string
}
...
export function TokenExpiryNotice({
  ...
  const variantStyles = {
    ...
    revoked: 'bg-[rgb(239,68,68)]/10 text-[rgb(239,68,68)] border-[rgb(239,68,68)]/30',
  }

  const displayMessage =
    message ??
    (variant === 'revoked'
      ? 'This link has been revoked and is no longer valid.'
  ...
  return (
    <div
      ...
      aria-live="polite"
    >
      {variant === 'error' || variant === 'revoked' ? (
        <AlertCircle className="h-4 w-4 shrink-0" aria-hidden />
      ) : variant === 'info' ? (
        <ShieldCheck className="h-4 w-4 shrink-0 text-[rgb(16,185,129)]" aria-hidden />
      ) : (
