...
projectsRouter.get('/projects', requireAuth, (req: Request, res: Response) => {
  ...
  const pendingByProject = db.prepare(
    `SELECT project_id, COUNT(*) as count FROM decisions
     WHERE deleted_at IS NULL AND status IN ('pending', 'in_review') GROUP BY project_id`
...
projectsRouter.post('/projects', requireAuth, (req: Request, res: Response) => {
  ...
  db.prepare(
    'INSERT INTO projects (id, name, description, account_id, studio_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)'
...
projectsRouter.get('/projects/:id', requireAuth, (req: Request, res: Response) => {
  const { id } = req.params
  const includeDeleted = req.query.includeDeleted === 'true'

  const row = db.prepare(
    'SELECT id, name, description, created_at, updated_at, deleted_at FROM projects WHERE id = ?'
  ).get(id) as { id: string; name: string; description: string | null; created_at: string; updated_at: string; deleted_at: string | null } | undefined

  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' })
  if (row.deleted_at && !includeDeleted) return res.status(404).json({ code: 'NOT_FOUND', message: 'Project not found' })

  const pendingCount = db.prepare(
    `SELECT COUNT(*) as c FROM decisions WHERE project_id = ? AND deleted_at IS NULL AND status IN ('pending', 'in_review')`
  ).get(id) as { c: number }
...
projectsRouter.patch('/projects/:id', requireAuth, (req: Request, res: Response) => {
  const { id } = req.params
  const { name, description } = req.body as { name?: string; description?: string }
...
projectsRouter.delete('/projects/:id', requireAuth, (req: Request, res: Response) => {
  const { id } = req.params
...
projectsRouter.post('/projects/:id/restore', requireAuth, (req: Request, res: Response) => {
  const { id } = req.params
