...
function getStudioIdForUser(_userId: string): string {
  return DEFAULT_STUDIO_ID
}

studiosRouter.get('/:id', requireAuth, (req: Request, res: Response) => {
  try {
    ...
    const studio = db.prepare(
      'SELECT id, name, branding_logo_url, branding_favicon_url, branding_invoice_accent_color, client_link_branding, default_currency, created_at, updated_at FROM studios WHERE id = ?'
    ).get(studioId) as {
      id: string
      name: string
      ...
      client_link_branding?: string | null
    ...
    const clientLinkBranding = studio.client_link_branding
      ? (JSON.parse(studio.client_link_branding) as { enabled?: boolean; customDomain?: string; domainValidated?: boolean })
      : null

    const sub = db.prepare(
      'SELECT plan_id, seats, seats_used, status, billing_cycle FROM subscriptions WHERE studio_id = ? ORDER BY updated_at DESC LIMIT 1'
    ).get(studioId) as {
      plan_id: string
      seats: number
      seats_used: number
    ...
    const teamMembers = db.prepare(
      `SELECT stm.id, stm.email, stm.role, stm.invited_at, stm.accepted_at, u.first_name, u.last_name
       FROM studio_team_members stm
       LEFT JOIN users u ON u.id = stm.user_id
       WHERE stm.studio_id = ?
       ORDER BY stm.invited_at DESC`
    ).all(studioId) as {
      id: string
      email: string
      role: string
...
studiosRouter.put('/:id', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    const studio = db.prepare(
      'SELECT id, name, branding_logo_url, branding_favicon_url, branding_invoice_accent_color, client_link_branding FROM studios WHERE id = ?'
    ).get(studioId) as {
      id: string
      name: string
      branding_logo_url: string | null
...
studiosRouter.get('/:id/defaults', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    const studio = db.prepare(
      'SELECT project_defaults FROM studios WHERE id = ?'
    ).get(studioId) as { project_defaults?: string | null } | undefined

    const defaults = studio?.project_defaults
      ? (JSON.parse(studio.project_defaults) as { reminderCadence?: string; exportFormats?: string[]; autoNotification?: boolean })
      : {}
...
studiosRouter.put('/:id/defaults', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    db.prepare('UPDATE studios SET project_defaults = ?, updated_at = datetime("now") WHERE id = ?').run(
...
studiosRouter.get('/:id/email-templates', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    const rows = db.prepare(
      'SELECT template_key, subject, body FROM studio_email_templates WHERE studio_id = ?'
    ).all(studioId) as { template_key: string; subject: string; body: string }[]
...
studiosRouter.put('/:id/email-templates/:key', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    ...
    const existing = db.prepare(
      'SELECT id FROM studio_email_templates WHERE studio_id = ? AND template_key = ?'
    ).get(studioId, key) as { id: string } | undefined

    if (existing) {
      db.prepare(
        'UPDATE studio_email_templates SET subject = ?, body = ?, updated_at = datetime("now") WHERE id = ?'
      ).run(subject, body, existing.id)
    } else {
      const id = crypto.randomUUID()
      db.prepare(
        `INSERT INTO studio_email_templates (id, studio_id, template_key, subject, body) VALUES (?, ?, ?, ?, ?)`
      ).run(id, studioId, key, subject, body)
    }
...
studiosRouter.post('/:id/team-members', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    const existing = db.prepare(
      'SELECT id FROM studio_team_members WHERE studio_id = ? AND email = ?'
    ).get(studioId, email.toLowerCase()) as { id: string } | undefined
    ...
    const id = crypto.randomUUID()
    db.prepare(
      `INSERT INTO studio_team_members (id, studio_id, email, role, invited_by) VALUES (?, ?, ?, ?, ?)`
    ).run(id, studioId, email.toLowerCase(), r, userId)

    const member = db.prepare(
      'SELECT id, email, role, invited_at, accepted_at FROM studio_team_members WHERE id = ?'
    ).get(id) as { id: string; email: string; role: string; invited_at: string; accepted_at: string | null }

    db.prepare(
      `INSERT INTO audit_logs (id, actor_id, action_type, target_type, target_id, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))`
    ).run(crypto.randomUUID(), userId, 'team_member_invited', 'studio_team_member', member.id, JSON.stringify({ studio_id: studioId, email: member.email, role: member.role }))
...
studiosRouter.put('/:id/team-members/:memberId', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    ...
    const member = db.prepare(
      'SELECT id, role FROM studio_team_members WHERE id = ? AND studio_id = ?'
    ).get(memberId, studioId) as { id: string; role: string } | undefined
    ...
    db.prepare(
      `INSERT INTO audit_logs (id, actor_id, action_type, target_type, target_id, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))`
    ).run(crypto.randomUUID(), userId, 'team_member_role_changed', 'studio_team_member', memberId, JSON.stringify({ studio_id: studioId, new_role: role }))

    const updated = db.prepare(
      'SELECT id, email, role, invited_at, accepted_at FROM studio_team_members WHERE id = ?'
    ).get(memberId) as { id: string; email: string; role: string; invited_at: string; accepted_at: string | null }
...
studiosRouter.delete('/:id/team-members/:memberId', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    const memberId = req.params.memberId
    ...
    const member = db.prepare(
      'SELECT id, email FROM studio_team_members WHERE id = ? AND studio_id = ?'
    ).get(memberId, studioId) as { id: string; email: string } | undefined
    ...
    db.prepare(
      `INSERT INTO audit_logs (id, actor_id, action_type, target_type, target_id, metadata, created_at) VALUES (?, ?, ?, ?, ?, ?, datetime("now"))`
    ).run(crypto.randomUUID(), userId, 'team_member_removed', 'studio_team_member', memberId, JSON.stringify({ studio_id: studioId, email: member.email }))
...
studiosRouter.get('/:id/audit-log', requireAuth, (req: Request, res: Response) => {
  try {
    const userId = (req as Request & { userId: string }).userId
    const studioId = req.params.id
    if (studioId !== getStudioIdForUser(userId)) {
    ...
    const rows = db.prepare(
      `SELECT al.id, al.action_type, al.metadata, al.created_at, u.first_name, u.last_name
       FROM audit_logs al
       LEFT JOIN users u ON u.id = al.actor_id
       WHERE al.metadata LIKE ?
       ORDER BY al.created_at DESC
       LIMIT 50`
    ).all(`%"studio_id":"${studioId}"%`) as {
      id: string
      action_type: string
