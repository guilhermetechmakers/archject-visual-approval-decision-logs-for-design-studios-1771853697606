 ...
 */
supportReportRouter.post('/support/report', (req: Request, res: Response) => {
  try {
    ...
    db.prepare(
      `INSERT INTO support_reports (id, user_id, email, attempted_path, message, user_agent, attachments, status, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'new', ?)`
    ).run(id, userId, emailVal, attemptedPathVal, msg, ua, att, now)
