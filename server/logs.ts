 ...
 */
logsRouter.post('/logs/error', (req: Request, res: Response) => {
  try {
    ...
    db.prepare(
      `INSERT INTO error_logs (id, level, type, message, attempted_path, referrer, user_id, session_id, correlation_id, meta, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, levelVal, typeVal, msg, pathVal, ref, uid, sid, cid, metaStr, timestamp ?? now)
