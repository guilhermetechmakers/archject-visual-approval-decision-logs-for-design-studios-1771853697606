...
libraryRouter.get('/projects/:projectId/files', requireAuth, (req: Request, res: Response) => {
  const { projectId } = req.params
  const search = (req.query.search as string)?.trim()
  const fileType = (req.query.fileType as string)?.trim()
  const archivedFilter = req.query.archived as string | undefined // 'true' | 'false' | 'all'
...
libraryRouter.post(
  ...
  (req: Request, res: Response) => {
    ...
    db.prepare(
      `INSERT INTO library_file_versions (id, file_id, version_number, filepath, size, uploaded_at, uploader_id, notes)
       VALUES (?, ?, 1, ?, ?, ?, ?, ?)`
...
libraryRouter.get('/projects/:projectId/files/:fileId', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  const row = db.prepare(
    'SELECT * FROM library_files WHERE id = ? AND project_id = ?'
  ).get(fileId, projectId) as Record<string, unknown> | undefined
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'File not found' })

  const version = db.prepare(
    'SELECT version_number FROM library_file_versions WHERE id = ?'
  ).get(row.current_version_id) as { version_number: number } | undefined
...
libraryRouter.put('/projects/:projectId/files/:fileId', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  ...
  if (isArchived !== undefined) {
    db.prepare('UPDATE library_files SET is_archived = ?, updated_at = ? WHERE id = ?').run(
      isArchived ? 1 : 0,
      now,
      fileId
    )
...
libraryRouter.post('/projects/:projectId/files/:fileId/restore', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  const row = db.prepare('SELECT id FROM library_files WHERE id = ? AND project_id = ?').get(fileId, projectId)
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'File not found' })
...
libraryRouter.post('/projects/:projectId/files/:fileId/restore', requireAuth, (req: Request, res: Response) => {
  ...
  const file = row as { id: string; is_archived: number }
  if (!file.is_archived) {
...
libraryRouter.delete('/projects/:projectId/files/:fileId', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  const row = db.prepare('SELECT id FROM library_files WHERE id = ? AND project_id = ?').get(fileId, projectId)
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'File not found' })
...
libraryRouter.post('/projects/:projectId/files/:fileId/restore', requireAuth, (req: Request, res: Response) => {
  ...
  const r = row as { deleted_at: string | null; is_archived: number }
...
libraryRouter.get('/projects/:projectId/files/:fileId/versions', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  ...
  const rows = db.prepare(
    'SELECT id, file_id, version_number, filepath, size, uploaded_at, uploader_id, notes FROM library_file_versions WHERE file_id = ? ORDER BY version_number DESC'
  ).all(fileId) as { id: string; file_id: string; version_number: number; filepath: string; size: number; uploaded_at: string; uploader_id: string | null; notes: string | null }[]
...
libraryRouter.post(
  ...
  (req: Request, res: Response) => {
    ...
    const libFile = db.prepare(
      'SELECT id, current_version_id FROM library_files WHERE id = ? AND project_id = ?'
    ).get(fileId, projectId) as { id: string; current_version_id: string } | undefined
    if (!libFile) return res.status(404).json({ code: 'NOT_FOUND', message: 'File not found' })

    if (!file) return res.status(400).json({ code: 'VALIDATION_ERROR', message: 'No file uploaded' })

    const max = db.prepare(
      'SELECT COALESCE(MAX(version_number), 0) + 1 as next FROM library_file_versions WHERE file_id = ?'
    ).get(fileId) as { next: number }
    const versionId = crypto.randomUUID()
    const now = new Date().toISOString()
    const relPath = `library/${projectId}/${file.filename}`
    ...
    db.prepare(
      `INSERT INTO library_file_versions (id, file_id, version_number, filepath, size, uploaded_at, uploader_id, notes)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(versionId, fileId, max.next, relPath, file.size, now, userId, notes ?? null)
    ...
    db.prepare(
      'UPDATE library_files SET current_version_id = ?, filepath = ?, size = ?, uploaded_at = ?, thumbnail_url = COALESCE(?, thumbnail_url), updated_at = ? WHERE id = ?'
    ).run(versionId, relPath, file.size, now, thumbUrl, now, fileId)
...
libraryRouter.post(
  ...
  (req: Request, res: Response) => {
    ...
    const version = db.prepare(
      'SELECT id, filepath, size FROM library_file_versions WHERE id = ? AND file_id = ?'
    ).get(versionId, fileId) as { id: string; filepath: string; size: number } | undefined
    ...
    const now = new Date().toISOString()
    db.prepare(
      'UPDATE library_files SET current_version_id = ?, filepath = ?, size = ?, updated_at = ? WHERE id = ?'
    ).run(versionId, version.filepath, version.size, now, fileId)
...
libraryRouter.get('/projects/:projectId/decisions/:decisionId/attachments', requireAuth, (req: Request, res: Response) => {
  ...
  const rows = db.prepare(
    `SELECT a.id, a.file_id, a.decision_id, a.notes, a.attached_at,
...
libraryRouter.post('/projects/:projectId/files/:fileId/attach', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { projectId, fileId } = req.params
  const { decisionId, notes } = req.body as { decisionId?: string; notes?: string }
...
libraryRouter.get('/projects/:projectId/files/:fileId/download', requireAuth, (req: Request, res: Response) => {
  const { projectId, fileId } = req.params
  const row = db.prepare(
    'SELECT f.filepath, f.filename FROM library_files f WHERE f.id = ? AND f.project_id = ?'
  ).get(fileId, projectId) as { filepath: string; filename: string } | undefined
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'File not found' })
