...
analyticsRouter.get('/projects', requireAuth, (_req: Request, res: Response) => {
  const rows = db.prepare('SELECT id, name FROM projects ORDER BY name').all() as { id: string; name: string }[]
  res.json({ projects: rows })
})

// GET /api/projects/:projectId/analytics (must be before /projects/:projectId)
analyticsRouter.get('/projects/:projectId/analytics', requireAuth, (req: Request, res: Response) => {
  const { projectId } = req.params
  const start = (req.query.start as string) || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10)
  ...
  try {
    ...
    const decisions = db.prepare(
      `SELECT id, title, status, type, created_at, decision_made_at, reviewer_id
       FROM decisions WHERE project_id = ? AND created_at >= ? AND created_at <= ?
       ORDER BY created_at ASC`
    ).all(projectId, `${start}T00:00:00.000Z`, `${end}T23:59:59.999Z`) as {
      id: string
      title: string
      status: string
      type: string
    ...
    const withApprovalTime = decisions
      ...
      .map((d) => ({
        ...d,
        approvalSeconds: Math.floor(
          (new Date(d.decision_made_at!).getTime() - new Date(d.created_at).getTime()) / 1000
        ),
      }))

    const medianApprovalSeconds =
      withApprovalTime.length > 0
        ? (() => {
    ...
    const heatmapByStatus: Record<string, Record<string, number>> = {}
...
analyticsRouter.get('/projects/:projectId', requireAuth, (req: Request, res: Response) => {
  const { projectId } = req.params
  const project = db.prepare('SELECT id, name, account_id, studio_id, created_at, updated_at FROM projects WHERE id = ?').get(projectId) as
    | { id: string; name: string; account_id: string | null; studio_id: string | null; created_at: string; updated_at: string }
    | undefined
...
function getDistributionBuckets(seconds: number[]): { bucket: string; count: number; minSeconds: number; maxSeconds: number }[] {
  const buckets = [
...
analyticsRouter.get('/analytics/decisions', requireAuth, (req: Request, res: Response) => {
  ...
  try {
    const countRow = db.prepare(`SELECT COUNT(*) as c FROM decisions d WHERE ${where}`).get(...params) as { c: number }
    const rows = db.prepare(
      `SELECT d.id, d.project_id, d.title, d.status, d.type, d.created_at, d.decision_made_at, d.reviewer_id,
...
// POST /api/exports
analyticsRouter.post('/exports', requireAuth, (req: Request, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO analytics_exports (id, requested_by, scope, filters_json, format, status)
     VALUES (?, ?, ?, ?, ?, 'queued')`
  ).run(id, userId, JSON.stringify(scope), filters ? JSON.stringify(filters) : null, fmt)

  setTimeout(() => {
    try {
      const projectId = scope.projectId
      const count = projectId
        ? (db.prepare('SELECT COUNT(*) as c FROM decisions WHERE project_id = ?').get(projectId) as { c: number }).c
        : 0
      db.prepare(
        'UPDATE analytics_exports SET status = ?, rows_count = ?, completed_at = ? WHERE id = ?'
      ).run('done', count, new Date().toISOString(), id)
    } catch {
...
analyticsRouter.get('/exports/:exportId/status', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { exportId } = req.params
  const row = db.prepare('SELECT * FROM analytics_exports WHERE id = ? AND requested_by = ?').get(exportId, userId) as
...
analyticsRouter.get('/exports/:exportId/download', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { exportId } = req.params
  const row = db.prepare('SELECT * FROM analytics_exports WHERE id = ? AND requested_by = ?').get(exportId, userId) as
...
analyticsRouter.get('/exports/:exportId/file', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { exportId } = req.params
  const row = db.prepare('SELECT * FROM analytics_exports WHERE id = ? AND requested_by = ?').get(exportId, userId) as
    | { status: string; scope: string; format: string }
    | undefined
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'Export not found' })
  const scope = (() => {
    try {
      return JSON.parse(row.scope || '{}') as { projectId?: string }
    } catch {
      return {}
    }
  })()
  ...
  const decisions = projectId
    ? db.prepare('SELECT * FROM decisions WHERE project_id = ?').all(projectId) as Record<string, unknown>[]
    : []
  const headers = decisions.length > 0 ? Object.keys(decisions[0] as object) : ['id', 'project_id', 'title', 'status', 'created_at']
  const csv = [headers.join(','), ...decisions.map((d) => headers.map((h) => JSON.stringify((d as Record<string, unknown>)[h] ?? '')).join(','))].join('\n')
  db.prepare('UPDATE analytics_exports SET status = ?, rows_count = ?, completed_at = ? WHERE id = ?').run(
    'done',
    decisions.length,
...
analyticsRouter.get('/exports', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const limit = Math.min(50, Math.max(1, parseInt((req.query.limit as string) || '10', 10)))
  const rows = db.prepare(
    `SELECT id, scope, filters_json, format, status, rows_count, created_at, completed_at
     FROM analytics_exports WHERE requested_by = ? ORDER BY created_at DESC LIMIT ?`
...
// CRUD /api/alerts
analyticsRouter.get('/alerts', requireAuth, (req: Request, res: Response) => {
  const scope = req.query.scope as string | undefined
  const scopeId = req.query.scopeId as string | undefined
  let where = 'created_by = ?'
  const params: unknown[] = [(req as Request & { userId: string }).userId]
  if (scope) {
  ...
  const rows = db.prepare(`SELECT * FROM alerts WHERE ${where} ORDER BY created_at DESC`).all(...params) as Record<string, unknown>[]
  res.json({
    alerts: rows.map((r) => ({
      id: r.id,
      name: r.name,
...
analyticsRouter.post('/alerts', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  ...
  const id = crypto.randomUUID()
  db.prepare(
    `INSERT INTO alerts (id, name, scope_type, scope_id, metric, operator, threshold_value, frequency_minutes, channels, enabled, created_by)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)`
  ).run(
    id,
    name,
...
analyticsRouter.put('/alerts/:id', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { id } = req.params
  const { enabled, ...updates } = req.body
  const row = db.prepare('SELECT id FROM alerts WHERE id = ? AND created_by = ?').get(id, userId)
  if (!row) return res.status(404).json({ code: 'NOT_FOUND', message: 'Alert not found' })
  const sets: string[] = []
  const params: unknown[] = []
...
analyticsRouter.delete('/alerts/:id', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { id } = req.params
  const r = db.prepare('DELETE FROM alerts WHERE id = ? AND created_by = ?').run(id, userId)
  if (r.changes === 0) return res.status(404).json({ code: 'NOT_FOUND', message: 'Alert not found' })
  res.json({ id, status: 'deleted' })
})

// GET /api/projects (for user's project selector)
analyticsRouter.get('/projects', requireAuth, (req: Request, res: Response) => {
  const rows = db.prepare('SELECT id, name FROM projects ORDER BY name').all() as { id: string; name: string }[]
  res.json({ projects: rows })
})
