...
leadsRouter.post('/demo', (req: Request, res: Response) => {
  try {
    ...
    db.prepare(
      `INSERT INTO leads (id, type, name, email, studio_name, phone, studio_size, message, utm_source, ip_address, status, created_at)
       VALUES (?, 'demo', ?, ?, ?, ?, ?, ?, ?, ?, 'new', ?)`
    ).run(
      id,
      name.trim().slice(0, 255),
      email.trim().slice(0, 320),
      typeof studioName === 'string' ? studioName.trim().slice(0, 255) : null,
      typeof phone === 'string' && phone.trim() ? phone.trim().slice(0, 50) : null,
      typeof studioSize === 'string' ? studioSize.trim().slice(0, 50) : null,
      typeof message === 'string' ? message.trim().slice(0, 2000) : null,
      typeof utmSource === 'string' ? utmSource.trim().slice(0, 255) : null,
      ip,
      now
    )
...
leadsRouter.post('/signup', (req: Request, res: Response) => {
  try {
    const ip = getClientIp(req)
    const limit = checkLeadsRateLimit(ip)
    if (!limit.allowed) {
    ...
    db.prepare(
      `INSERT INTO leads (id, type, email, utm_source, ip_address, status, created_at)
       VALUES (?, 'signup', ?, ?, ?, 'new', ?)`
    ).run(
      id,
      email.trim().slice(0, 320),
...
brandingRouter.get('/:studioId/preview', (req: Request, res: Response) => {
  try {
