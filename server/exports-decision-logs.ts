...
function runExportJobWorker(jobId: string) {
  ...
  if (!row || (row.type !== 'EXPORT_PDF' && row.type !== 'EXPORT_CSV')) return

  const steps = [
    { name: 'Queued', status: 'completed' as const },
    { name: 'Preparing', status: 'in_progress' as const },
    { name: 'Generating', status: 'pending' as const },
    { name: 'Signing', status: 'pending' as const },
    { name: 'Packaging', status: 'pending' as const },
    { name: 'Ready', status: 'pending' as const },
  ]
...
exportsDecisionLogsRouter.post('/exports/create', requireAuth, (req: Request, res: Response) => {
  ...
  db.prepare(
    `INSERT INTO exports (id, project_id, created_by, status, type, job_id, meta, download_count)
     VALUES (?, ?, ?, 'queued', ?, ?, ?, 0)`
  ).run(
    exportId,
    projectId,
...
exportsDecisionLogsRouter.get('/exports/:jobId/status', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  ...
  if (row.status === 'COMPLETED' && row.result_urls) {
    try {
      const urls = JSON.parse(row.result_urls) as { url?: string }[]
      const first = urls[0]
      if (first?.url) {
  ...
  if (row.status === 'FAILED' && row.error) {
    try {
      const err = JSON.parse(row.error) as { message?: string }
      error = err.message
    } catch {
      error = row.error
    }
...
exportsDecisionLogsRouter.get('/exports/:jobId/download', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { jobId } = req.params
...
exportsDecisionLogsRouter.get('/branding/profiles', requireAuth, (_req: Request, res: Response) => {
  const rows = db.prepare(
    'SELECT id, name, branding_logo_url, branding_invoice_accent_color FROM studios WHERE id IS NOT NULL'
  ).all() as { id: string; name: string; branding_logo_url: string | null; branding_invoice_accent_color: string | null }[]
  ...
  const profiles = rows.map((r) => ({
    ...
    headerMarkdown: null as string | null,
    isActive: true,
  }))
...
exportsDecisionLogsRouter.post('/branding/profiles', requireAuth, (req: Request, res: Response) => {
  const { id, name, logoUrl, colorHex, domain, font, headerMarkdown, isActive } = req.body as {
    id?: string
    name?: string
    logoUrl?: string | null
  ...
  const existing = db.prepare('SELECT id FROM studios WHERE id = ?').get(studioId) as { id: string } | undefined
  ...
  const row = db.prepare('SELECT id, name, branding_logo_url, branding_invoice_accent_color FROM studios WHERE id = ?').get(studioId) as {
    id: string
    name: string
...
exportsDecisionLogsRouter.get('/decision-logs/:decisionId/audit', requireAuth, (req: Request, res: Response) => {
...
exportsDecisionLogsRouter.get('/exports/history', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  ...
  const rows = db.prepare(
    `SELECT e.id, e.project_id, e.created_by, e.created_at, e.status, e.type, e.job_id, e.meta, e.file_key, j.status as job_status, j.result_urls
     FROM exports e
     LEFT JOIN jobs j ON j.id = e.job_id
     WHERE ${where} ORDER BY e.created_at DESC LIMIT ?`
  ).all(...params) as {
    id: string
    project_id: string
  ...
  const exports = rows.map((r) => {
    ...
    const decisionIds = (meta.decisionIds as string[]) ?? []
