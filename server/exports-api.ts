...
const EXPORT_TYPES = ['csv', 'pdf', 'merged_pdf', 'signed_pdf', 'zip'] as const

export const exportsRouter = Router()

exportsRouter.post('/exports', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { projectId, decisionIds = [], types = ['pdf'], options = {} } = req.body as {
    projectId: string
    decisionIds: string[]
    types: string[]
    options?: { includeImages?: boolean; brandingId?: string; signer?: Record<string, unknown> }
  }
  ...
  const validTypes = types.filter((t: string) => EXPORT_TYPES.includes(t as (typeof EXPORT_TYPES)[number]))
  ...
  for (const type of validTypes) {
    ...
    db.prepare(
      ...
      type,
      jobId,
      JSON.stringify({ decisionIds, options })
    )
...
exportsRouter.get('/exports/:exportId', requireAuth, (req: Request, res: Response) => {
...
exportsRouter.get('/exports/:exportId/download', requireAuth, (req: Request, res: Response) => {
  const userId = (req as Request & { userId: string }).userId
  const { exportId } = req.params
  ...
  res.setHeader(
    'Content-Disposition',
    `attachment; filename="${row.file_name ?? `export-${exportId.slice(0, 8)}.pdf`}"`
  )
