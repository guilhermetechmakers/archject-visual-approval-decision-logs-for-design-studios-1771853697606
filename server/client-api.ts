...
function getTokenPayload(token: string): { projectId: string; decisionIds: string[]; tokenId?: string; exp?: number; expiresAt?: string } | null {
  ...
  try {
    ...
    tokenRow = db.prepare(
      `SELECT id, project_id, decision_ids, expires_at FROM client_tokens WHERE token_hash = ?`
    ).get(tokenHash) as TokenRow | undefined
  }
  ...
  try {
    const decoded = jwt.verify(token, JWT_SECRET) as { projectId?: string; decisionIds?: string[]; exp?: number }
    return {
...
function recordAnalytics(tokenId: string, eventType: 'view' | 'comment' | 'approve' | 'export', decisionId?: string, req?: Request): void {
  try {
    ...
    db.prepare(
      `INSERT INTO portal_analytics (id, token_id, event_type, decision_id, ip_hash, user_agent, locale)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).run(id, tokenId, eventType, decisionId ?? null, ipHash, userAgent, locale)
...
clientRouter.get('/client/:token/decisions', (req: Request, res: Response) => {
...
clientRouter.get('/client/:token/decision/:decisionId', (req: Request, res: Response) => {
  const token = req.params.token
...
clientRouter.get('/client/:token/decision/:decisionId/history', (req: Request, res: Response) => {
  const token = req.params.token
  const decisionId = req.params.decisionId
  const payload = getTokenPayload(token)
  if (!payload) {
  ...
  let rows: { id: string; action: string; performed_by: string | null; performed_at: string; details: string | null }[]
  try {
    rows = db.prepare(
...
clientRouter.get('/client/:token/decision/:decisionId/comments', (req: Request, res: Response) => {
  const token = req.params.token
  const decisionId = req.params.decisionId
  const payload = getTokenPayload(token)
  if (!payload) {
...
clientRouter.post('/client/:token/decision/:decisionId/comment', (req: Request, res: Response) => {
  const token = req.params.token
  const decisionId = req.params.decisionId
  ...
  try {
    db.prepare(
      `INSERT INTO portal_comments (id, decision_id, parent_comment_id, author_id, author_name, content, created_at, client_token_id)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(id, decisionId, parentCommentId ?? null, 'client', 'Client', content.trim(), now, token.slice(0, 20))
  } catch (e) {
...
clientRouter.post('/client/:token/decision/:decisionId/export', (req: Request, res: Response) => {
  const token = req.params.token
  const decisionId = req.params.decisionId
  const payload = getTokenPayload(token)
  if (!payload) {
...
clientRouter.post('/client/:token/decision/:decisionId/followup', (req: Request, res: Response) => {
  const token = req.params.token
  const decisionId = req.params.decisionId
...
clientRouter.get('/client/:token/notifications', (req: Request, res: Response) => {
  const token = req.params.token
  const payload = getTokenPayload(token)
  if (!payload) {
    return res.status(404).json({ code: 'INVALID_TOKEN', message: 'Invalid or expired token' })
...
clientRouter.get('/client/:token/token/status', (req: Request, res: Response) => {
  const token = req.params.token
  const payload = getTokenPayload(token)
  if (!payload) {
...
clientRouter.get('/client/:token/branding', (req: Request, res: Response) => {
  const token = req.params.token
  const payload = getTokenPayload(token)
  if (!payload) {
    return res.status(404).json({ code: 'INVALID_TOKEN', message: 'Invalid or expired token' })
  ...
  const project = db.prepare(
    `SELECT p.id, p.studio_id, s.branding_logo_url, s.branding_invoice_accent_color
     FROM projects p
     LEFT JOIN studios s ON s.id = p.studio_id
     WHERE p.id = ?`
  ).get(payload.projectId) as { id: string; studio_id: string | null; branding_logo_url: string | null; branding_invoice_accent_color: string | null } | undefined
  if (!project) {
    return res.json({
      logoUrl: null,
      primaryColor: '#0052CC',
  ...
  const studio = project.studio_id
    ? db.prepare('SELECT branding_logo_url, branding_invoice_accent_color FROM studios WHERE id = ?').get(project.studio_id) as { branding_logo_url: string | null; branding_invoice_accent_color: string | null } | undefined
    : null
  return res.json({
